...
function transformUtil.checkIfTransform(value: Transform.Transform?): boolean
	return if value and value.__type == "Transform" then true else false
end
...
function transformUtil.getTransform(value: Type.PositionData | Transform.Transform): Transform.Transform?
	...
	return if isTransform then value else nil
end
