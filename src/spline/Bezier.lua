...
export type Bezier = typeof(setmetatable({} :: BezierProperties & Spline.Spline, Bezier))
...
function Bezier.sampleRotation(self: Bezier, t: number): number | Quaternion.Quaternion
	...
	if self.rotationType == "Quaternion" then
		...
		return quaternionInterpolation.cumulative(
			self.rotations :: { Quaternion.Quaternion },
			cummulativeBasisPolynomials
		)
	end

	error("Rotation Type is either a Angle or Quaternion")
end
