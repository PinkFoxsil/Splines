...
export type Quaternion = typeof(setmetatable({} :: QuaternionProperties, Quaternion))
...
function Quaternion.fromRotationMatrix(matrix: Matrix.Matrix<number>): Quaternion
	...
	if trace > 0 then
		...
		return Quaternion.new(-quat[1], -quat[2], -quat[3], w)
	end
end
...
function Quaternion.fromAngleAxis(angleRad: number, axis: Vector3): Quaternion
	...
	return Quaternion.new(s * axis.X, s * axis.Y, s * axis.Z, cos(halfAngle))
end
...
function Quaternion.differentiateAngularVelocity(self: Quaternion, nextQuat: Quaternion, deltaTime: number)
	return Quaternion.toScaledAngleAxis((nextQuat * self ^ -1):abs() / deltaTime)
end

function Quaternion.differentiateAngularVelocityApproximate(self: Quaternion, nextQuat: Quaternion, deltaTime: number)
	return Quaternion.toScaledAngleAxisApproximate((nextQuat * self ^ -1):abs() / deltaTime)
end
...
function Quaternion.fastSlerp(q0: Quaternion, q1: Quaternion, t: number): Quaternion
	...
	local cT = sign
		* t
		* (1 + bT[1] * (1 + bT[2] * (1 + bT[3] * (1 + bT[4] * (1 + bT[5] * (1 + bT[6] * (1 + bT[7] * (1 + bT[8]))))))))
	local cD = d
		* (1 + bD[1] * (1 + bD[2] * (1 + bD[3] * (1 + bD[4] * (1 + bD[5] * (1 + bD[6] * (1 + bD[7] * (1 + bD[8]))))))))
...
function Quaternion.exp(vect: Vector3): Quaternion
	...
	return Quaternion.new(vect.X * s, vect.Y * s, vect.Z * s, cos(theta))
end
...
function Quaternion.multiplyByScalar(q0: Quaternion, val: number): Quaternion
	return Quaternion.new(q0.x * val, q0.y * val, q0.z * val, q0.w * val)
end
...
function Quaternion.__add(self: Quaternion, b: Quaternion): Quaternion
	return Quaternion.new(self.x + b.x, self.y + b.y, self.z + b.z, self.w + b.w)
end
